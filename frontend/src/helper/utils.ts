...
export function MapArrayToQuery(
  ...
  filterAnimal.forEach((filter, index) => {
    if (filter.animal_type != "" && filter.cage_size != "") {
      queryParams += `animals[${index}].animal_type=${encodeURIComponent(
        filter.animal_type
      )}&`;
      queryParams += `animals[${index}].cage_size=${encodeURIComponent(
        filter.cage_size
      )}&`;
    }
  });
  queryParams =
    queryParams.slice(-1) === "&" ? queryParams.slice(0, -1) : queryParams;
  // console.log("Query Params:", queryParams);
  return queryParams;
}

export function UpdateImageArray(
  newImages: UploadRes[],
  images: string[] | undefined,
  strImage: string | undefined
): string[] {
  if (strImage == "") {
