...
const RoomDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchCageRoomType = async () => {
      try {
        const response = await fetch(`${BASE_API}/cageroom/type/${profileId}`, {
          method: "GET",
          headers: {
  ...
  const fetchCageRoomById = async (cageRoomId: number) => {
    if (cageRoomId) {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`${BASE_API}/cageroom/${cageRoomId}`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/json",
  ...
  const handleRemoveFacility = (facility: string) => {
    const updatedFacilities = (filteredCageData.facility_array || []).filter(
      (f: string) => f !== facility
    );
    setFilteredCageData({
      ...filteredCageData,
  ...
  const handleRemoveImage = (index: number) => {
    const updatedImages = (filteredCageData.image_array || []).filter(
      ...
      <div className="flex justify-center pb-10">
        <div className="flex w-3/4 items-center flex-col gap-y-2">
          ...
          <div className="pt-10 space-x-1">
            ...
            <button className="bg-nextstep border shadow-lg h-10 w-20 rounded-md text-white">
              Edit
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-y-6 p-6 w-full max-w-4xl mx-auto">
        <div className="relative mb-10">
          <div className="absolute top-0 left-0 flex justify-center gap-x-4">
            <button
              className="text-gray-500 bg-egg border shadow-lg py-1 px-2 rounded-lg"
              onClick={() => navigate("/hotel/edit")}
            >
              Hotel Detail
            </button>
            <button className="text-white text-xl bg-nextstep border shadow-lg p-3 rounded-lg">
              Cage Detail
            </button>
          </div>
        </div>
          ...
          <div className="flex gap-x-4">
            ...
            >
              ...
              {selectedAnimal &&
                cageRoomData
                  .find(
                    (animal: { animal_type: string }) =>
                      animal.animal_type === selectedAnimal
                  )
          ...
          <div className="bg-bg p-4 rounded-lg shadow-lg flex flex-col gap-y-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                {(filteredCageData.image_array || []).map(
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {(filteredCageData.facility_array || []).map(
                  ...
                  <div className="bg-[#FFFBF5] rounded-lg p-6 shadow-lg max-w-sm w-full">
                    <p className="text-[#5E4126] text-center font-medium mb-4">
                      ...
                      <span className="text-sm text-gray-600">
                        Notice that you can’t recover it.
                      </span>
                    </p>
