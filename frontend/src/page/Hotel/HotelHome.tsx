...
export default function HotelHome() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_API}/profile/${id}/hotel`, {
      method: "GET",
      headers: {
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (profileId) {
      fetch(`${BASE_API}/cageroom/all/${profileId}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (profileId) {
      fetch(`${BASE_API}/hotel/review/${profileId}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
    ...
    <div className="flex justify-center bg-bg  pb-10">
      <div className="flex w-3/4 items-center flex-col gap-y-2 bg-bg">
        ...
        <div className="pt-10 space-x-1 text-white">
          <button className="bg-nextstep border shadow-lg h-10 w-20 rounded-md">
            View
          </button>
          <button
              ...
              {rooms && rooms.length > 0 ? (
                rooms.map((room: any, index: any) => (
                  <div key={`room-${index}`} className=" flex flex-col">
                    ...
                    >
                      <div className="basis-1/3 bg-cover  w-72  pt-5 pl-5">
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-y-5 text-xl ">
                          <div className="flex items-center gap-1">
                            <p className="text-center font-bold bg-yellow p-1 w-10">
                              {" "}
                              {room.size}{" "}
                            </p>
                            <p>
                              Size {room.height} X {room.width} X {room.lenth}{" "}
                            </p>
                          </div>

                          <p>Capacity: {room.quantity || "N/A"}</p>
                          <p>
                            Facility: {room.facility || "Standard facilities"}
                          </p>
                        </div>
                      </div>
                      <div className="basis-1/3 space-y-5 pl-5 pt-4 flex flex-col items-end pr-5 gap-y-5">
                        ...
                        <p>
                          Free cancel before {new Date().toLocaleDateString()}
                        </p>
                      </div>
                    </div>
        ...
        <div className="flex flex-col bg-bg w-full mt-10">
          <div className="flex space-x-5">
            <h1 id="review" className="text-2xl">
              Review
            </h1>
