...
export default function MyProfile() {
    const [profile, setProfileData] = useState<any>(null); // State to store profile data
    ...
    const [currentTab, setCurrentTab] = useState<string>("MyProfile"); // State to manage current tab
    const [formData, setFormData] = useState<any>(null); // State to manage form data
    const navigate = useNavigate();
    const token = localStorage.getItem('token');
    const id = localStorage.getItem('userId');
    const [image, setImage] = useState<any>(null); // State to manage profile image
    const [petImage, setPetImage] = useState<any>(null); // State to manage pet image
    const [pet, setPetData] = useState<any>(null); // State to manage pet data
    const [allPet, setAllPet] = useState<any>(null); // State to manage all pets data
    ...
    const [animalType, setAnimalType] = useState<string>(""); // State to manage animal type

    // Handle input changes for pet form
    const handleInputChangePet = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        const { name, value } = e.target;
        if (name === "animal_type") {
            setAnimalType(value); // Update animal type when selected
        }
        setPetData((prevPet: any) => ({
            ...prevPet,
            [name]: value,
                            ...
                            <div className="flex mt-10   gap-x-20 pl-20 items-center ">
                                {isEditing ? (
                                    ...
                                    <div className="flex overflow-hidden rounded-full ">
                                        <img
                                            src={formData.image_profile || image} // Use new or current image
                                            alt="Profile Image"
