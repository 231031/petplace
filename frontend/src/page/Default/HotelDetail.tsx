...
function HotelDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_API}/hotel/review/${profile_id}`, {
      method: "GET",
      headers: {
    ...
    <div className="flex justify-center bg-bg  pb-10">
      <div className="flex w-3/4 items-center flex-col gap-y-10 bg-bg">
        ...
        <div className="pt-10 text-black">
          <div className="bg-white grid grid-cols-5 gap-1 p-2 rounded-md shadow-md">
            <button
              ...
              onClick={() => scrollToSection(hotelRef)}
            >
              Hotel
            </button>
            <button
            ...
            <button
              className="h-10 w-20 rounded-md "
              onClick={() => scrollToSection(roomRef)}
            >
              Room
            </button>
            <button
        ...
        <div className="flex flex-col w-full h-[36rem]  gap-y-5 ">
          <h1 ref={hotelRef} id="hotel" className="text-4xl">
            {hotel.name}
          </h1>
        ...
        <div className="flex w-full h-72 mt-10">
          ...
          <div className="flex flex-col gap-y-5 w-full  ">
            <h1 ref={detailRef} id="detail" className="text-2xl">
              Detail
            </h1>
        ...
        <div className="flex flex-col w-full gap-y-5 pb-5">
          <h1 ref={facilityRef} id="facility" className="text-2xl">
            {" "}
            Facility
          </h1>
        ...
        <div className="flex flex-col w-full ">
          <h1 ref={roomRef} id="room" className="text-2xl mb-5">
            Cage
          </h1>
                ...
                <div>
                  {hotel.cages.map((cage: Cage, index: number) => (
                    <div className="flex flex-col">
                      <div
                        key={index}
                        className={`flex mx-5 mt-5 p-3 shadow shadow-gray-400 h-80 ${
                      ...
                      >
                        ...
                        <div
                          className="basis-1/3  flex flex-col space-y-5 pl-5 pt-4 cursor-pointer "
                          onClick={() => handleRoomClick(index)}
                        >
                          <h1 className="text-2xl font-semibold">
                            {cage.cage_type} : {cage.animal_type}
                          </h1>
                          <div>
                            <div className="flex flex-col gap-y-5">
                              <div className="flex gap-2 ">
                                <p className="bg-yellow text-center font-bold w-10 p-1">
                                  {cage.size}
                                </p>
                                <p className="flex items-center">
                                  Size {cage.height} X {cage.width} X{" "}
                                  {cage.lenth}{" "}
                                </p>
                              </div>
                        ...
                        <div className="basis-1/3  space-y-5 pl-5 pt-4 flex flex-col items-end pr-5 gap-y-5">
                          ...
                          <div className="flex space-x-2">
                            <button
                              className="w-fit px-2 h-8 bg-bg rounded-full shadow"
                              onClick={() => handleAddFavorite(cage)}
                            >
                              Add Favorite
                            </button>
                            <button
                              className="w-fit px-2 h-8  rounded-full bg-yellow hover:bg-navbar"
        ...
        <div className="flex flex-col bg-bg w-full mt-10">
          <div className="flex space-x-5">
            <h1 ref={reviewRef} id="review" className="text-2xl">
              Review
            </h1>
            {Array.from({ length: 5 }, (_, i) => (
              <span key={i} className={` text-2xl`}>
                {i < Math.floor(hotel.avg_review) ? (
                  ...
                  <i
                    className="fa-solid fa-star-half-alt 0"
                    style={{ color: "#DBA54D" }}
                  ></i> // Half star
                ) : (
                  <i
                  ...
                  <div className="flex">
                    <div className="flex  gap-x-2 flex-col w-8/12 ">
                      <h1 className="text-xl font-bold">
                        Name:{" "}
                        {
                          review[index].animal_hotel_services[0].animal_user
                            .user.first_name
                        }
                      </h1>
                      <div className="flex gap-x-2">
                        {Array.from({ length: 5 }, (_, i) => (
                          <span key={i} className={`text-yellow-500 text-lg`}>
                            {i < Math.floor(item.review_rate) ? (
                              ...
                              <i
                                className="fas fa-star-half-alt 0"
                                style={{ color: "#DBA54D" }}
                              ></i> // Half star
                            ) : (
                              <i
                      ...
                      <p className="mr-4 ">
                        {item.review_detail ||
                          " At {hotel.name}, we believe your pets deserve a vacation too Our pet hotel offers a safe, comfortable, and enriching environment for your furry family members, with amenities designed specifically for both cats and dogs."}
                      </p>
                    </div>
                    <div className="flex flex-col  w-4/12 justify-end h-full">
                      ...
                      <div className="mt-2 mb-10">
                        <CarouselCage
                          images={review[index].review_image_array || []}
                        />
                      </div>
                    </div>
                  </div>
